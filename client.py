...
class Client:
    ...
    def get_sch_ids(self,sch_abbr: str) -> list[int]:
        if sch_abbr not in Client.sch_ids:
            res = requests.get(f"https://house.nfu.edu.tw/{sch_abbr}")
    ...
    def get_house_data(self,sch_ids: list[int]) -> dict:
        '''
    ...
    def get_response(self, sch_abbr: str) -> dict:
        sch_ids = self.get_sch_ids(sch_abbr)
        return self.get_house_data(sch_ids)
